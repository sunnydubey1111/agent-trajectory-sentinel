...
\title{Detecting Failures in LLM Agents:\\
       What Lightweight Runtime Verification Can and Cannot See}
...
The release also carries a standalone contributions document that states each
scientific result in the project --- the laws, the measured boundaries, the
verification and repair outcomes, the negative results --- next to the artifact
it is computed from and the bound it carries, so a reader can audit the set
rather than reconstruct it from the paper.
