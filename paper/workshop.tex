...
$+0.40$ at $\ge9$, and this law predicts its own failure region on two external
corpora we did not build --- on AFTraj-2K, where 94\% of failures end within
eight steps of onset, pooled detection is $0.048$ exactly as the law forecasts,
while at $\ge9$ steps it is $0.509$. Second, an \emph{observability boundary}:
...
healthy runs only, with no failure labels and no second model. We are
interested less in its headline accuracy than in its \emph{boundary} --- what
such a verifier structurally cannot see, and what to pair with it there.
...
\item \textbf{A horizon law with external validation.} The temporal monitor's
      ...
      and holds on two corpora built by other groups (\S\ref{sec:external}).
...
independent thresholds do, and give a non-degradation property at serving time
--- no stream's evidence can be numerically cancelled by another's.
...
AUROC (mean $-0.014$) and on all 8 in detection rate (mean $-0.140$). The
hybrid is the right default when the regime is unknown, and the wrong choice
when it is known.
...
horizon law says where it will not fire, and held on two corpora built by other
groups. Where it structurally cannot see --- a wrong-but-well-formed value ---
