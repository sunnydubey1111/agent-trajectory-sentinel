...
(\texttt{alarm\_repair.csv}, $n=25$ live episodes) every one of the 18
behavioural alarms was followed by a repair attempt and no run that did not alarm
was interrupted, but \texttt{goal\_drift} is the only class a retry fixes (2 of
5). Where the tool layer itself is broken a retry fetches the same broken result:
